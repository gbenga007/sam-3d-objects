...
"""
Fine-tune the metric-scale heads on OmniNOCS NOCS-Real275 — MULTI-GPU VERSION.

This is the HuggingFace Accelerate-enabled variant of `finetune_metric_scale.py`.
Launch with: accelerate launch --num_processes N finetune_metric_scale_multi_gpu.py ...
For SLURM clusters: see `scripts/train_mixed_v1_multi_gpu.sh`.

Single-GPU also works (just `python finetune_metric_scale_multi_gpu.py ...`)
because Accelerator gracefully no-ops when not launched via accelerate launch.

Effective batch size = (per-GPU batch) × num_processes. With our batch_size=1
constraint (24GB A10 / SAM3D forward pass VRAM), 16 GPUs = effective batch 16.
Scale the LR accordingly via --lr (e.g. sqrt scaling: lr × sqrt(num_gpus)).
...
- MetricScaleHead, MetricScaleDecoder, optimizer, and train DataLoader are
  prepared via `accelerator.prepare()` (Accelerate handles DDP wrap and
  distributed sampling).
- The SLAT cross-attn and SS decoder parameters live inside `pipeline.models`,
  which is a complex non-standard nn.Module structure. Wrapping the full
  pipeline.models with DDP breaks the pipeline's forward routing. Instead we
  manually all-reduce gradients on those parameter groups before optimizer.step.
- Heldout evaluation runs only on the main process (small dataset, simpler
  than sharding+gather).
- Checkpoint saves only happen on the main process and call
  `accelerator.unwrap_model()` before extracting state dicts.
"""
...
def _all_reduce_grads(params: list, world_size: int) -> None:
    """Average gradients across all ranks for parameters NOT wrapped in DDP.

    Used for the SLAT cross-attn and SS decoder parameters that live inside
    pipeline.models[...]: those modules cannot be wrapped with accelerator.prepare()
    without breaking the pipeline's internal forward routing, so we manually
    sync their gradients here before optimizer.step().
    """
    if world_size <= 1:
        return
    for p in params:
...
def save_metric_checkpoint(
    ...
    output_path = Path(path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    # Unwrap DDP/FSDP wrappers so state_dict keys are clean (no "module." prefix).
    payload = {
        "metric_scale_head": _unwrap_ddp(scale_head).state_dict(),
        "metric_scale_decoder": _unwrap_ddp(scale_decoder).state_dict(),
        "args": vars(args),
        "epoch": epoch,
        "metrics": metrics,
    }
...
def main() -> None:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Multi-GPU init via HuggingFace Accelerate.
    # When launched with `accelerate launch --num_processes N`, this sets up
    # DDP across N processes. When run as plain `python`, num_processes=1 and
    # Accelerator is a no-op wrapper.
    # ──────────────────────────────────────────────────────────────────────
    ...
    if is_main:
        print(
            f"Accelerate: num_processes={world_size}, "
            f"device={accelerator.device}, mixed_precision=bf16"
        )

    if args.device.startswith("cuda") and not torch.cuda.is_available():
        raise RuntimeError("CUDA was requested but torch.cuda.is_available() is false.")
    # Override args.device with accelerator's device so each rank uses its own GPU.
    ...
    # Seed all ranks identically for model init; Accelerate handles dataloader
    # sampling randomization separately via its distributed sampler.
    accel_set_seed(args.seed)

    # Only rank-0 talks to Weights & Biases.
    wandb_run = init_wandb(args) if is_main else None
    ...
    if args.load_feature_cache:
        ...
        # accelerator.prepare wraps with DistributedSampler so each rank sees
        # a unique slice of the data and shuffling stays in sync across ranks.
    ...
    # DDP-wrap the small trainable heads. SLAT cross-attn + SS decoder live
    # inside pipeline.models and are gradient-synced manually via
    # _all_reduce_grads() after backward() (see training loop below).
    scale_head = accelerator.prepare(scale_head)
    scale_decoder = accelerator.prepare(scale_decoder)
    ...
    if args.eval_only:
        ...
        # --eval-only is a single-process operation; only run on rank 0 to
        # avoid duplicate file writes / wandb logs.
        if is_main:
    ...
    if args.cache_latents or args.load_feature_cache:
        ...
        for epoch in range(args.epochs):
            ...
            for offset in progress:
                ...
                # SLAT cross-attn and SS decoder params are NOT DDP-wrapped
                # (they live inside pipeline.models); sync their grads manually.
                _all_reduce_grads(slat_cross_attn_params, world_size)
                _all_reduce_grads(ss_decoder_params, world_size)
        ...
        if eval_dataset is not None and args.eval_every and resume_start_epoch == 0:
            ...
            if heldout_metrics is not None and is_main:
            ...
            for step_idx, batch in enumerate(progress):
                ...
                try:
                    ...
                    if torch.isnan(loss) or torch.isinf(loss):
                        ...
                        continue
                    accelerator.backward(loss)
                    # Manual gradient all-reduce for params NOT in DDP-wrapped modules.
                    # Heads (scale_head/decoder) are DDP-synced automatically by
                    # accelerator.backward(); these need explicit syncing.
                    _all_reduce_grads(slat_cross_attn_params, world_size)
                    _all_reduce_grads(ss_decoder_params, world_size)
    ...
    if is_main:
        ...
        if wandb_run is not None:
            wandb_run.finish()
    # Ensure all ranks reach the end together.
    accelerator.wait_for_everyone()
