#!/usr/bin/env bash
# Native FM v2 (2026-06-23): FIXED-SEED shape sample (canon deterministic per item, train==eval).
...
CONDA_PREFIX=/opt/conda/envs/sam3d /opt/conda/envs/sam3d/bin/python \
  ...
  --resume-from artifacts/metric_scale/checkpoints/native_fm_v2_resume.pt \
  --output artifacts/metric_scale/checkpoints/native_fm_v2.pt \
  --best-output artifacts/metric_scale/checkpoints/native_fm_v2_best.pt \
  --metrics-output artifacts/metric_scale/metrics/native_fm_v2_eval.jsonl \
  --manifest-output artifacts/metric_scale/manifests/native_fm_v2_manifest.json \
  --wandb --wandb-mode offline --wandb-entity reformed-tulip \
  --wandb-project sam3d-metric-scale --wandb-run-name native_fm_v2
